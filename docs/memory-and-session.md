...
- **`memory_extractor` 双写**：在进入 workflow LLM 前将 **本轮传入子 Agent 的整段文本** 交给 **`Extractor.Extract`**（OpenAI 兼容 Chat Completions）；当抽取命中记忆时，结果 **追加写入** `memory/<UTC-yyyy-mm>/<UTC-yyyy-mm-dd>.md`（空结果不落盘），并由上游库 **持久化结构化条目** 到 InstructionRoot 下的 **`memory/structured_sqlite.db`**（WAL SQLite + FTS5，与 `memory/<yyyy-mm>/` 并列）。
- **主回合 `structuredmem` 抽取（同步，`on_respond` 后）**：抽取输入 **为当轮 JSONL 文件** `runs/<agent_type>/<correlation_id>.jsonl`（内含 `run_start` / `user_message` / **`tool_call` / `tool_result`** / `assistant_message` / `run_complete`；子 Agent 为 `runs/<agent>/<parent_corr>__<sub_run>.jsonl`。**不再**写入聚合 `runs.jsonl`。抽取失败或文件缺失时回落为 `User: …\\nAssistant: …`。抽取结果会先做 **轻量过滤**（丢弃典型瞬时时间类 `transient`）并在同一批次内对 **PMO + AI Infra 会议数量** 这类冲突做 **审计记录 +  journal 侧移除重复条目**（SQLite 侧仍以 extractor 写入为准，journal 用于人类可读核对）。仅 **用户明确表达的称呼偏好**（例如包含“请叫我/叫我/preferred name/call me”）才会 **提升到 `MEMORY.md`**（受 2048 字节上限约束）；不把助手自我命名默认当作长期 profile。
...
| 2026-05-05 | `memory_extractor`：`Extractor.Extract` + 追加当日 md（空结果不写）；SQLite **`memory/structured_sqlite.db`**；MemoryRecall **FTS 优先** + 可选路径摘要；主回合 `structuredmem`：**仅** **`runs/<agent>/<correlation_id>.jsonl`**（含 tool）；无聚合 `runs.jsonl`；`read_run_journal` **full** 为目录内多文件合并；journal 侧冲突审计；`MEMORY.md` 自动区块仅提升 **用户明确的称呼偏好**；专用 memory/skill/instruction 文件工具收敛为 **`read_file` / `write_file`**；工具失败 **`[tool_error]`** 回流模型 |

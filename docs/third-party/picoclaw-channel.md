...
> **目的**：对标 PicoClaw 分层与接口的**调研笔记**；**不写实现代码**。  
...
| 横切能力 | 未实现草案：**`ctx` 透传**、**`SinkRegistry`/`SinkFactory`** 见 [`architecture-modularity-simplification.md`](../architecture-modularity-simplification.md) | Manager：限流、重试、占位/打字等 |
...
4. **与 `SinkFactory.NewSink(ctx, in)` 的关系**：oneclaw 侧 **`SinkFactory` 等为未实现草案**（见 [`architecture-modularity-simplification.md`](../architecture-modularity-simplification.md)）；PicoClaw 更偏「渠道实例 + Manager 状态表」，合并时需统一「有状态出站」的归属（Factory vs Channel 实例字段）。

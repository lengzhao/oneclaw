...
- **Memory / Session / 检查点** 与 **结构化记忆库**：Eino 核心不负责「对话存盘格式」；与 **`github.com/lengzhao/memory`**、oneclaw 文件真源的边界见 **[memory-and-session.md](memory-and-session.md)**。
...
**Memory / Session（业务层）与 `CheckPointStore`**：

- 官方快速入门 **第三章** 中的 `Session` / `Store` / JSONL 是 **示例工程**（[`eino-examples` chatwitheino/mem](https://github.com/cloudwego/eino-examples/tree/main/quickstart/chatwitheino/mem)），用于说明如何把 **历史 `[]*schema.Message`** 持久化后再交给 `Runner`；**不是** `go get eino` 自带的固定 API。
- **`RunnerConfig.CheckPointStore`** 面向 **中断 / 恢复**（HITL、检查点 ID），与「按轮追加的对话日志」不同；详见 [memory-and-session.md](memory-and-session.md) §1 表 **C** 行与 §2 末段。
- **结构化记忆库（SQLite）**：可选模块 **`github.com/lengzhao/memory`** 与 Eino 解耦；接入方式见 [memory-and-session.md](memory-and-session.md) §3。
...
| 会话消息持久化 / 外部记忆库 | 业务层实现或 `lengzhao/memory`；见 [memory-and-session.md](memory-and-session.md) |
...
| 2026-05-03 | §8 增补 Memory/Session/`CheckPointStore`/`lengzhao/memory` 指针；导读改为链接 [memory-and-session.md](memory-and-session.md)；§10 映射表增加会话持久化行 |

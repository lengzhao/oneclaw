...
本文定义 Claw **回合级编排工作流** 的 YAML 形状：以 **有向无环图（DAG）** 为主模型（节点 + 边），与 Eino **Compose Graph** 对齐；线性 **`steps`** 仅作**语法糖**（展开为链状图）。满足 [requirements.md](requirements.md) FR-FLOW-02 / FR-FLOW-05，并与 [eino-md-chain-architecture.md](eino-md-chain-architecture.md)、[architecture.md](architecture.md) §2.1 一致。
...
> 若仅需「先回复用户再后台演进」，典型形状为：`main → respond`，再由 `respond` 同时连到 `memory_extract` 与 `skill_gen`，且后两者 **`async: true`**。
...
| `agent` | **独立一次 ADK 运行**（子 Agent、**回合后**记忆/Skills 演进、其它后台管线） | **`agent_type`**（必填，Catalog id）, `workspace`, … |
| `memory_extract_llm` | 事实抽取（可封装 `agent`） | `staging_only: true` |
| `skill_suggest_llm` | Skills 草案 | `staging_only: true` |
...
nodes:
  gate:
    use: if
    params:
      expr: "context.flags.evolution_enabled && !context.evolution_suppressed"
  mem:
    use: agent
    async: true
...
edges:
  ...
  - { from: gate, to: mem, branch: true }
  - { from: gate, to: skip_mem, branch: false }
```

---

**是否只要「最后多一个 Agent、且异步」？** **可以。** 主路径仍是 `… → on_respond`；在图上从 **`on_respond` 再引一条（或多条）出边** 到 **`use: agent`** 的节点，并在该节点上设 **`async: true`**，即表示 **用户侧可见回复已走完后**，再 **后台** 拉记忆抽取等专用 `agent_type`（多任务则多节点、多边，见 §4.3、§8）。**不必**把记忆抽取塞进 `adk_main` 同一次 ReAct 里。需要 **开关演进** 时在前面加 **`if`**（§6.1）。

**FR-FLOW-05**：`memory_extractor` / `skill_generator` 等须在 Catalog 中 **`suppress_post_turn_evolution: true`**；宿主应对图做 **静态或运行时** 校验。

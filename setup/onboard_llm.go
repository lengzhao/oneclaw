...
var BuiltinLLMPresets = []LLMProviderPreset{
	...
	{"5", "Moonshot", "moonshot", "moonshot", "https://platform.moonshot.cn/", "https://api.moonshot.cn/v1", "key_files/moonshot.json", "moonshot-v1-8k", false},
...
func RunLLMProviderOnboard(out io.Writer, in io.Reader, userDataRoot string, noBrowser bool) (config.ModelProfile, string, error) {
	...
	if !found {
		return config.ModelProfile{}, "", fmt.Errorf("onboard: invalid choice %q", choice)
	}
	...
	if preset.Custom {
		...
		if base == "" {
			return config.ModelProfile{}, "", fmt.Errorf("onboard: base_url required")
		}
	...
	if preset.ProfileID == "ark" && strings.TrimSpace(preset.ModelDefault) == "" {
		...
		if ep == "" {
			return config.ModelProfile{}, "", fmt.Errorf("onboard: ark model (endpoint id) required")
		}
	...
	if apiKey == "" {
		return config.ModelProfile{}, "", fmt.Errorf("onboard: empty API key")
	}
	...
	if modelID == "" {
		return config.ModelProfile{}, "", fmt.Errorf("onboard: empty model id")
	}

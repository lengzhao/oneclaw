...
**Far-field scope:** think **across sessions and days** — dedupe and merge near-duplicate **episodic** bullets in the digest; **standing rules** belong in AGENT / `.oneclaw/rules` / skills / MEMORY.md via **`write_behavior_policy`**, not in the digest. Prefer a **compact** rules file; **never** state that a file contains rules unless you actually read that file in this pass.
**Skills:** When logs or transcripts show **repeated tool use** that should collapse into **one guided workflow**, or **user corrections** that define reusable procedure, **try** to **create or update** **`.oneclaw/skills/<name>/SKILL.md`** via **`write_behavior_policy`** (focused body: trigger, steps, pitfalls). Use the digest for **one-off facts**; prefer **skills** for **repeatable playbooks** so future turns invoke them instead of rediscovering the same tool loop.

...
// ExecInput matches the exec tool JSON schema and is used by [RunExecInWorkspace].
...
// RunExecInWorkspace runs a shell command with cwd = workspaceRoot using the same policy and limits as the exec tool ([InferExec]).
func RunExecInWorkspace(ctx context.Context, workspaceRoot string, in ExecInput) (string, error) {
	root := strings.TrimSpace(workspaceRoot)
	if root == "" {
		return "", fmt.Errorf("%s: workspace root required", NameExec)
	}
...
func InferExec(workspaceRoot string) (tool.InvokableTool, error) {
	...
	return utils.InferTool(NameExec, "Run a shell command with workspace as cwd. Default disabled; set tools.exec.enabled and allow prefix list in config (deny substrings optional). Output capped.",
		func(ctx context.Context, in ExecInput) (string, error) {
			return RunExecInWorkspace(ctx, absRoot, in)
		})
}

...
type cronScheduleIn struct {
	AtSeconds    int    `json:"at_seconds,omitempty" jsonschema:"description=One-shot: seconds from now"`
	AtRFC3339    string `json:"at_rfc3339,omitempty" jsonschema:"description=One-shot: RFC3339 wall time in the future"`
	EverySeconds int    `json:"every_seconds,omitempty" jsonschema:"description=Recurring: interval in seconds (uses @every)"`
...
type cronToolIn struct {
	...
	Message  string          `json:"message,omitempty" jsonschema:"description=add: user message text when the job fires"`

...
执行 **`go run ./cmd/oneclaw -init`** 会在 **`$HOME/.oneclaw/`** 生成或更新 **`config.yaml`**（无则写入完整内置模板；已有则只补上模板里缺失的键、不覆盖你的配置），并复制 **`AGENT.md`**、**`memory/MEMORY.md`**、**`skills/skill-creator/`**（Anthropic [skills](https://github.com/anthropics/skills/tree/main/skills/skill-creator) 规程整包 vendored）、**`agents/skill-generator.md`**（子 agent：`run_agent` / **`skill-generator`**）、**`MAINTAIN_SCHEDULED.md`** / **`MAINTAIN_POST_TURN.md`**（维护 system 提示模板，与内置一致；已存在则不覆盖）等，再创建记忆目录；再按需编辑密钥与渠道。

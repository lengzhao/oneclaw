...
// inboundAttachmentMsgs are full user message bodies (one message each).
// userLine is the primary user turn text (non-empty after engine validation).
func AppendTurnUserMessages(msgs *[]openai.ChatCompletionMessageParamUnion, memAgentMd, memRecall, inboundMeta string, inboundAttachmentMsgs []string, userLine string) {
	if msgs == nil {
		return
	}
	...
	for _, block := range inboundAttachmentMsgs {
